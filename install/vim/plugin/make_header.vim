...
function!	Insert_header_42_add_info_asm(begin, end, nul_line, setline)
	let l:line = ";    " . a:begin . ": " . strftime("%Y/%m/%d %H:%M:%S") . " by " . s:author
	let l:cmd = "echo " . s:author . " | wc -c | awk '{print $1}' | tr -d '\n'"
	let l:len = system(l:cmd)
	let l:space = 17 - l:len
...
function!	Insert_header_42_add_mail_asm()
	let l:mail = system("echo $MAIL | tr -d '\n'")
	let l:line = ";    By: " . s:author . " <" . l:mail . ">"
	let l:cmd = "echo " . s:author . l:mail . " | wc -c | awk '{print $1}' | tr -d '\n'"
	let l:len = system(l:cmd)
...
function!	Insert_header_42_add_file_asm()
	let l:line = ";    "
	let l:file = expand('%:f')
	let l:cmd = "/usr/bin/basename " . l:file . " | tr -d '\n'"
	let l:file = system(l:cmd)
...
function!	Insert_header_42_asm()
let s:author = system("echo $USER | tr -d '\n'")
	call append(0, "; **************************************************************************** ;")
	call append(1, ";                                                                              ;")
	call append(2, ";                                                        :::      ::::::::     ;")
	call Insert_header_42_add_file_asm()
	call append(4, ";                                                    +:+ +:+         +:+       ;")
	call Insert_header_42_add_mail_asm()
	call append(6, ";                                                +#+#+#+#+#+   +#+             ;")
	call Insert_header_42_add_info_asm("Created", " #+#    #+#              ;", 7, 0)
	call Insert_header_42_add_info_asm("Updated", "###   ########.fr        ;", 8, 0)
	call append(9, ";                                                                              ;")
	call append(10, "; **************************************************************************** ;")
	call append(11, "")
	unlet s:author
...
function!	Update_header_42_asm()
	let s:author = system("echo $USER | tr -d '\n'")
	let l:pattern = ";    Updated:"
	let l:line = getline(9)
	if match(l:line, l:pattern) != -1
		call Insert_header_42_add_info_asm("Updated", "###   ########.fr        ;", 9, 1)
	endif
	unlet s:author
endfunction
...
function!	Update_header_42()
	...
	unlet s:author
	call Update_name_file_header_42()
endfunction

function!	Update_name_file_header_42()
	let l:pattern = "/*  "
